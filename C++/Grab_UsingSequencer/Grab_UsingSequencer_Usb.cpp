...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (GenApi::IsAvailable(camera.SequencerMode))
        {
            ...
            // quarter height
            camera.Height.SetValue(camera.Height.GetInc() * (increments / 4) + camera.Height.GetMin());
            ...
            camera.SequencerSetNext.SetValue(curSet + incSet);
            // half height
            camera.Height.SetValue(camera.Height.GetInc() * (increments / 2) + camera.Height.GetMin());
            camera.SequencerSetSave.Execute();
            ...
            // full height
            camera.Height.SetValue(camera.Height.GetInc() * increments + camera.Height.GetMin());
            camera.SequencerSetSave.Execute();
